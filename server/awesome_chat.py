...
if inference_mode != "huggingface":
    Model_Server = "http://" + config["local_inference_endpoint"]["host"] + ":" + str(
        config["local_inference_endpoint"]["port"])
...
def send_request(data):
    ...
    import logging
    import json
    title = data['prompt'].split(':')[0]
    with open(f"{INDEX}-{title}.json", 'w') as f:
        f.write(json.dumps(data, ensure_ascii=False))
    logging.error(f"{json.dumps(data, ensure_ascii=False)}")
    response = requests.post(endpoint, json=data, headers=HEADER, proxies=PROXY)
    if "error" in response.json():
        return response.json()
    logger.debug(response.text.strip())

    with open(f"{INDEX}-{title}-response.json", 'w') as f:
...
def run_task(input, command, results, openaikey=None):
    ...
    if task.endswith("-text-to-image") or task.endswith("-control"):
        if inference_mode != "huggingface":
            ...
            record_case(success=False, **{"input": input, "task": command,
            ...
            return False
    elif task in ["summarization", "translation", "conversational", "text-generation",
...
def chat_huggingface(messages, openaikey=None, return_planning=False, return_results=False):
    ...
    if "error" in task_str:
        record_case(success=False, **{"input": input, "task": task_str,
    ...
    try:
        ...
        response = chitchat(messages, openaikey)
        record_case(success=False,
                    **{"input": input, "task": task_str, "reason": "task parsing fail", "op": "chitchat"})
